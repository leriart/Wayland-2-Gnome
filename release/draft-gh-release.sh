#!/usr/bin/env bash
# ── GitHub Release Draft Helper ──────────────────────────────────────────────
# Run this after a successful build (cargo build --release) to stage the
# release assets and print the markdown body for the GitHub release.
#
# Usage: ./release/draft-gh-release.sh
...
$(grep "^## \[${VERSION}\]" CHANGELOG.md -A 9999 | sed -n '/^## \[/ { /^## \['"${VERSION}"'\]/ { p; d; }; q; }; p')
...
wayland-2-gnome-${VERSION}-linux-${ARCH}/
  wayland-2-gnome           (${SIZE} compressed)
  README.md
  LICENSE
  config.toml
